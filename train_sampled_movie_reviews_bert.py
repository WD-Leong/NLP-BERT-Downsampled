...
model_ckpt_dir  = "TF_Models/movie_reviews_bert_down"
train_loss_file = "train_loss_movie_reviews_bert_down.csv"

# Load the data. #
tmp_pkl_file = "/home/Data/imdb_movie_review/"
tmp_pkl_file += "train_movie_reviews_bert_data.pkl"
...
tmp_pkl_file = "/home/Data/imdb_movie_review/"
tmp_pkl_file += "test_movie_reviews_bert_data.pkl"
with open(tmp_pkl_file, "rb") as tmp_load_file:
...
# Filter out the data. #
train_data = [
    (x, y, z) for x, y, z in data_tuple if len(z) != 0]
...
print("Building the BERT Downsampled Model.")
start_time = time.time()

bert_model = bert.BERTClassifier(
    num_class, num_layers, num_heads, 
    hidden_size, ffwd_size, vocab_size, 
    seq_length+3, ker_sz, rate=1.0-p_keep)
bert_optimizer = tf.keras.optimizers.Adam(
    beta_1=0.9, beta_2=0.98, epsilon=1.0e-9)

elapsed_time = (time.time()-start_time) / 60
print("BERT Downsampled Model Built", 
      "(" + str(elapsed_time) + " mins).")

# Print the model summary. #
tmp_zero = np.zeros(
    [sub_batch, seq_length+3], dtype=np.int32)
...
print("-" * 50)
print("Training the BERT Downsampled Network", 
...
while n_iter < maximum_iter:
    ...
    for n_index in range(batch_size):
        ...
        tmp_i_tok = train_data[tmp_index][2]
        n_tokens  = len(tmp_i_tok)
        
        # To make the model more robust, a random permutation #
        # of the sequence is taken, regardless of whether the #
        # text length exceeds the predefined sequence length. #
        idx_permute = list(
            np.random.permutation(n_tokens))
        
        #n_sample = np.random.randint(
        #    0, high=n_tokens)
        #n_sample = min(max(1, n_sample), seq_length)
        n_sample = min(n_tokens, seq_length)
        
        idx_sample = list(sorted(
            idx_permute[:n_sample]))
        tmp_i_idx = [word_2_idx.get(
            tmp_i_tok[x], UNK_token) for x in idx_sample]
        ...
        tmp_p_idx = [CLS_token] + tmp_i_idx
        if n_tokens > seq_length:
            tmp_p_idx += [TRU_token, EOS_token]
        else:
            tmp_p_idx += [EOS_token]
            ...
            for tmp_n in range(curr_batch):
                tmp_input = [
                    word_2_idx.get(x, UNK_token) for \
                        x in test_data[id_st+tmp_n][2]]
                
                # Truncate if the length is longer. #
                n_input  = len(tmp_input)
                idx_perm = list(
                    np.random.permutation(n_input))
                
                idx_sample = list(sorted(
                    idx_perm[:seq_length]))
                tmp_i_idx  = [
                    tmp_input[x] for x in idx_sample]
                
                tmp_toks = [CLS_token]
                if n_input > seq_length:
                    tmp_toks = tmp_i_idx
                    tmp_toks += [TRU_token, EOS_token]
                else:
                    tmp_toks += tmp_i_idx + [EOS_token]
                n_decode = len(tmp_toks)
                
                tmp_test_tokens[tmp_n, :n_decode] = tmp_toks
                del tmp_toks, tmp_input, idx_perm
                del tmp_i_idx, n_input, n_decode
...
# TO DO: Apply test time augmentation. #

# Print the evaluation report. #
print("Generating evaluation report.")

pred_report = classification_report(
    test_labels, eval_labels, target_names=["Neg", "Pos"])

tmp_result_file = "movie_review_validation_report.txt"
